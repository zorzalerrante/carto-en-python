...
This is a modified version by Eduardo Graells-Garrido (egraells@udd.cl), for internal use
within a custom library for a Data Science course. 

Particularly, we have included code from this commit:

https://github.com/mawesi/smopy/commit/75e58bc9503f17b5d53dc8fceff91920c0754a89

We have also changed the default configuration in order to use Wikimedia tiles and
retina tiles.
...
TILE_SIZE = 512
MAXTILES = 128
TILE_SERVER = 'https://maps.wikimedia.org/osm-intl/{z}/{x}/{y}@2x.png'
TILE_FOLDER = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'wikimedia_tiles')
...
def fetch_tile(x, y, z):
    ...
    Return a PIL image.
    """        
    file_path = os.path.join(TILE_FOLDER, 'smopy_cache_{}_{}_{}.png'.format(x, y, z))
...
def fetch_map(box, z):
    ...
    img = Image.new('RGB', (int(sx*TILE_SIZE), int(sy*TILE_SIZE)))
